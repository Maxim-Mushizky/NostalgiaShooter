...
class RayCasting:
    ...
    def ray_cast(self):
        ...
        for ray in range(settings.NUM_RAYS):
            ...
            depth = self._remove_fishbowl_effect(depth, ray_angle)
            # projection
    ...
    def _remove_fishbowl_effect(self, depth, ray_angle) -> float:
        return depth * np.cos(self.game.player.angle - ray_angle)

    def _adjust_color_by_depth(self, depth: float,
                               full_scale: int = 255,
                               depth_pow: float = 5,
                               offset: float = 0.00002) -> list[float]:
        return [full_scale / (1 + depth ** depth_pow * offset)] * 3
